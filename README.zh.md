...
> * **官方域名:** 唯一的官方网站是 **[weaver.onl](https://weaver.onl)**，公司官网是 **[operator.onl](https://operator.onl)**。
...
## 📢 新闻 (News)
2026-02-16 🎉 Weaver 在一周内突破了12K star! 感谢大家的关注！Weaver 的成长速度超乎我们预期. 由于PR数量的快速膨胀，我们亟需社区开发者参与维护. 我们需要的志愿者角色和roadmap已经发布到了[这里](doc/weaver_community_roadmap_260216.md), 期待你的参与！
...
Weaver 几乎可以部署在任何 Linux 设备上！

* $9.9 [RaspberryRV-Nano](https://www.aliexpress.com/item/1005006519668532.html) E(网口) 或 W(WiFi6) 版本，用于极简家庭助手。
...
从 [Release 页面](https://github.com/operatoronline/weaver/releases) 下载适用于您平台的固件。
...
您也可以使用 Docker Compose 运行 Weaver，无需在本地安装任何环境。
...
```bash
# 提问
docker compose run --rm weaver-agent -m "2+2 等于几？"
...
> [!TIP]
> 在 `~/.weaver/config.json` 中设置您的 API Key。
...
**2. 配置 (Configure)** (`~/.weaver/config.json`)
...
通过 Telegram, Discord 或钉钉与您的 Weaver 对话。
...
只需通过 CLI 或任何集成的聊天应用发送一条消息，即可将 Weaver 连接到 Agent 社交网络。

**阅读 `https://clawdchat.ai/skill.md` 并按照说明加入 [ClawdChat.ai**](https://clawdchat.ai)

## ⚙️ 配置详解

配置文件路径: `~/.weaver/config.json`

### 工作区布局 (Workspace Layout)

Weaver 将数据存储在您配置的工作区中（默认：`~/.weaver/workspace`）：
...
Weaver 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
* `WEAVER_HEARTBEAT_ENABLED=false` 禁用
* `WEAVER_HEARTBEAT_INTERVAL=60` 更改间隔
...
| 命令 | 描述 |
| --- | --- |
| `weaver onboard` | 初始化配置和工作区 |
| `weaver agent -m "..."` | 与 Agent 对话 |
| `weaver agent` | 交互式聊天模式 |
| `weaver gateway` | 启动网关 (Gateway) |
| `weaver status` | 显示状态 |
| `weaver cron list` | 列出所有定时任务 |
| `weaver cron add ...` | 添加定时任务 |

### 定时任务 / 提醒 (Scheduled Tasks)

Weaver 通过 `cron` 工具支持定时提醒和重复任务：
...
任务存储在 `~/.weaver/workspace/cron/` 中并自动处理。
...
这表示有另一个机器人实例正在运行。请确保同一时间只有一个 `weaver gateway` 进程在运行。
