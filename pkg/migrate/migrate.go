...
func Run(opts Options) (*Result, error) {
	...
	weaverHome, err := resolveWeaverHome(opts.WeaverHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, weaverHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to Weaver")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, weaverHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, weaverHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(weaverHome, "config.json"),
				Description: "convert OpenClaw config to Weaver format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(weaverHome)
...
func Execute(actions []Action, openclawHome, weaverHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, weaverHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, weaverHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing Weaver config: %w", err)
		}
...
func resolveWeaverHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("WEAVER_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".weaver"), nil
}
